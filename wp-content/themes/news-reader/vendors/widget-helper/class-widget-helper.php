...
if ( ! class_exists(' NewsReader_Widget_Helper' ) ) {
	...
	* @since 1.0.0
	*/
	class NewsReader_Widget_Helper extends WP_Widget {
			...
			switch ( $type ) {
				...
				case 'image':
					...
					<div class="widget-field-image">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
						<input type="button" class="button button-primary wh-image-picker" value="<?php esc_attr_e( 'Upload', 'news-reader' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'news-reader' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'news-reader' ); ?>" />
						...
						<input type="button" class="button button-secondary btn-image-remove" value="<?php esc_attr_e( 'Remove', 'news-reader' ); ?>" <?php echo ( empty( $value ) ) ? 'style="display:none;"' : ''; ?> />
