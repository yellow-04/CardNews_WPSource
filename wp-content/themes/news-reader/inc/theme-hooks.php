...
if( !function_exists('newsreader_header_before') ){
	/**
	* Add top header before.
	*
	* @since 1.0.0
	*/
	function newsreader_header_before(){
	?>
    <header id="home" class="header">
    <?php
	}
}
add_action( 'newsreader_header_container', 'newsreader_header_before',10 );


if( !function_exists('newsreader_header_top_bar') ){
	/**
	* Add Header Top Bar.
	*
	* @since 1.0.0
	*/
	function newsreader_header_top_bar(){
            ...
            <div class="container">
              <div class="row"> 
                ...
                <div class="col-md-5 pull-right">
                  <?php
                    /**
                     * Hook - newsreader_social_profile.
                     *
                     * @hooked newsreader_social_profile - 10
                     */
					 if( get_theme_mod( 'newsreader_theme_options_socialheader','1') == 1 ):
                    	 do_action( 'newsreader_social_profile' );
					 endif;
                    ?> 
...
add_action( 'newsreader_header_container', 'newsreader_header_top_bar',11 );


if( !function_exists('newsreader_header_container') ){
	/**
	* Add top header.
	*
	* @since 1.0.0
	*/
	function newsreader_header_container(){
	?>
    	  <div class="container">
            <div class="row navbar-header">
              <div class="col-md-4"> 
                  ...
                  <!-- /Toggle Button --> 
					...
					<?php
                    /**
                     * Hook - newsreader_site_branding.
                     *
                     * @hooked newsreader_site_branding - 10
                     */
                    do_action( 'newsreader_site_branding' );
                    ?> 
...
add_action( 'newsreader_header_container', 'newsreader_header_container',12 );

if ( ! function_exists( 'newsreader_add_primary_navigation' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function newsreader_add_primary_navigation() {
		?>
...
add_action( 'newsreader_header_container', 'newsreader_add_primary_navigation',13 );


if( !function_exists('newsreader_header_after') ){
	/**
	* Add top header before.
	*
	* @since 1.0.0
	*/
	function newsreader_header_after(){
	?>
    <div class="clearfix"></div>
        </header>
    <?php
...
add_action( 'newsreader_header_container', 'newsreader_header_after',14 );


if ( ! function_exists( 'newsreader_site_branding' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function newsreader_site_branding() {
	?>
	 <?php
...
add_action( 'newsreader_site_branding', 'newsreader_site_branding' );


if ( ! function_exists( 'newsreader_social_profile' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function newsreader_social_profile() {
	?>
...
add_action( 'newsreader_social_profile', 'newsreader_social_profile' );
...
if ( ! function_exists( 'newsreader_footer_container_copy_right' ) ) :
	...
	function newsreader_footer_container_copy_right() {
		$newreader_options = get_theme_mod( 'newsreader_theme_options' );
	?>
    	<div class="clearfix"></div>
        <hr>
        ...
        <!-- Copyright -->
        <p class="copyright">
        	<?php echo esc_html( $newreader_options['footer']['copyright'] );?> <br/>
        
       	 <a href="<?php /* translators:straing */ echo esc_url( esc_html__( 'https://wordpress.org/', 'news-reader' ) ); ?>"><?php /* translators:straing */  printf( esc_html__( 'Proudly powered by %s', 'news-reader' ), 'WordPress' ); ?></a>
        <span class="sep"> | </span>
        <?php
        printf( /* translators:straing */  esc_html__( 'Theme: %1$s by %2$s.', 'news-reader' ), 'News Green', '<a href="' . esc_url( __( 'https://edatastyle.com', 'news-reader' ) ) . '" target="_blank">' . esc_html__( 'eDataStyle', 'news-reader' ) . '</a>' ); ?>
        </p>
...
if ( ! function_exists( 'newsreader_posted_on' ) ) :
	...
	?>
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'news-reader' ) );
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'news-reader' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
			echo '</li>';
		}
		?>
        </ul>
...
if ( ! function_exists( 'newsreader_page_post_title_render' ) ) :
	...
	function newsreader_page_post_title_render() {
		...
		if ( is_home() ) {
			...
			echo '<h1 class="title">';
			printf( esc_html__( 'Search Results for: %s', 'news-reader' ),  get_search_query() );
			echo '</h1>';
		} elseif ( is_404() ) {
			echo '<h1 class="title">';
			esc_html_e( '404 Error', 'news-reader' );
			echo '</h1>';
		}elseif( is_shop() ){
